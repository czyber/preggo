...
export const usePregnancyStore = defineStore('pregnancy', () => {
  ...
  const activePregnancies = computed(() => {
    return pregnancies.value.filter(pregnancy => pregnancy.is_active)
  })
  ...
  const currentWeek = computed(() => {
    ...
    return currentPregnancy.value.current_week
  })
  
  const dueDate = computed(() => {
    if (!currentPregnancy.value) return null
    return currentPregnancy.value.due_date
  })
  
  const babyInfo = computed(() => {
    if (!currentPregnancy.value) return []
    return currentPregnancy.value.babies
  })
  ...
  async function fetchPregnancies() {
    ...
    try {
      ...
      const activePregnancy = pregnancies.value.find(p => p.is_active)
      if (activePregnancy) {
  ...
  async function createPregnancy(pregnancyData: PregnancyCreate) {
    try {
      const api = useApi()
